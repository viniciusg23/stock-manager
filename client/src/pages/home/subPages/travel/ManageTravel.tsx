...
function ManageTravel() {
    ...
    const handleRemovePurchase = (purchaseId: number) => {
        const updatedArray = purchasedProducts.filter((purchase) => purchase.id !== purchaseId);
    ...
    useEffect(() => {
        const total = purchasedProducts.reduce((accumulator, purchased) => accumulator + purchased.total, 0);
        setActualCost(total);
    }, [purchasedProducts]);
                ...
                >
                    ...
                    >
                        ...
                        <NewPurchaseForm
                            ...
                            handleUniquePriceChange={(event: React.ChangeEvent<HTMLInputElement>) => setUniquePrice(event.target.value)}
                            quantity={quantity}
                            handleQuantityChange={(event: React.ChangeEvent<HTMLInputElement>) => setQuantity(event.target.value)}
                            total={total}
                            handleTotalChange={(event: React.ChangeEvent<HTMLInputElement>) => setTotal(event.target.value)}
                            handleProductSelect={(event: React.ChangeEvent<{}>, value: any | null) => setSelectedProduct(value)}
                    ...
                    >
                        ...
                        >
                            ...
                            <Button
                                disabled={purchasedProducts.length > 0 ? false : true}
                                ...
                                onClick={(e) => {
                                    ...
                                    const pdfBody = { 
                                        travelName: travelName, 
                                        travelCost: travelCost,
                                        produtos: purchasedProducts, 
                                        valorTotal: totalValue 
                                    };

                                    PDFGenerator(pdfBody);
                                }}
                            >
